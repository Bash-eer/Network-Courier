...
import { API } from "../../../views/Modules/Operations/checkIn/const";
...
const CheckInTab = {
  ...
  actions: {
    ...
    async loadJobsForCheckin({ commit, state }) {
      try {
    ...
    async updateJobsForCheckIn(_, payload) {
      try {
    ...
    PrintMovementChart({ state, commit }, data) {
      return _post(`${API.PRINT_MCHART}/${data}`)
        .then(async (response) => {
          return response;
        })
        .catch((err) => {
