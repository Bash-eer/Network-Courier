<template>
  ...
  >
    ...
    >
      <template #body="slotProps">
          ...
          >
            <div class="flex heading">
              ...
              {{ slotProps.data?.building_name }}, #{{
                slotProps.data?.floor_no
              }}-{{ slotProps.data?.unit_no }},
            </div>
            <div class="flex heading">
        ...
        <span v-if="col.header == 'Members'">
          ...
          <AvatarGroup>
            <span
              v-for="(avathar, index) of slotProps.data.contacts"
              :key="index"
            >
              <Avatar :image="avathar.profile_url" />
            </span>
          </AvatarGroup>
          <!-- <GroupedAvatars
            :data="
    ...
    <Column
      v-if="tableName != 'QuotationTabSubTable' && selectedCustomer != 'Adhoc'"
      :expander="
    ...
    <template class="nestedTable" #expansion="slotProps">
      <!-- <div v-if="selectedCustomer == 'History'">
        <DataTableExpansion
          tableName="fuelChargesTableExpansion"
      ...
      <div v-else> -->
      <!-- <DataTable :value="slotProps.data.cost_center" responsiveLayout="scroll">
        <Column
          class="contentText"
        ...
        <Column field="state" header="Location"></Column>
        <Column field="zone_id" header="Return trip charges"></Column>
        <Column headerStyle="width:4rem">
          <template #body="slotProps1">
            <Buttons
              label="Select"
              button_class="button-Class btn-go-property"
              @click="tableClick(slotProps1, slotProps)"
            />
          </template>
        </Column>
      </DataTable>
      ...
      <DataTable :value="slotProps.data.cost_center" responsiveLayout="scroll">
        ...
        <Column field="address" header="Address"></Column>
        <Column field="state" header="Location"></Column>
        <Column header="Return trip charges">
          <template #body="slotProps">
            {{
              slotProps.data.express_cost_center.site_office_charges[0]
                ?.return_trip_charge ?? ""
            }}
          </template>
        </Column>
...
export default {
  ...
  data() {
    return {
      ...
      rowSelect: null,
      filters: null,
      overlayDataPrompt: CustomerLaunchTableOverlayData,
      tableColumn: CSRActivityCollectionSlipColumnFields,
    };
  },
  created() {
