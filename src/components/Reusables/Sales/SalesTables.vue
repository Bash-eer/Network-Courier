<template>
  ...
  " :class="tableClass" :showGridlines="false" :stripedRows="true" :rowClass="rowClass" selectionMode="single"
    @rowSelect="onRowSelect" @rowUnselect="onRowUnselect" v-model:selection="rowSelect"
    v-model:expandedRows="expandedRows" :dataKey="dataKey" @rowExpand="onRowExpand" @rowCollapse="onRowCollapse"
    responsiveLayout="scroll" :value="tableData" :paginator="tableName == 'QuotationTabSubTable' ? false : true"
    :rows="10"
    paginatorTemplate="CurrentPageReport FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown"
    :rowsPerPageOptions="[10, 20, 30]" currentPageReportTemplate="Showing {first}-{last} of {totalRecords} results">
    <!-- <Column v-if="tableName == 'QuotationTabSubTable'" /> -->
    <Column v-if="tableName != 'QuotationTabSubTable'" :expander="
      tableName == 'QuotationTabSubTable' ||
      tableName == 'SalesTab' ||
    ...
    <Column v-for="col of tableColumns" :field="col.field" :header="col.header" :key="col.field" :showFilterMenu="
    ...
    " :sortable="tableName != 'QuotationTabSubTable' ? true : false">
      <template #body="slotProps">
        <!-- TEMPLATE : ACTIONS -->
        <span v-if="col.header == 'Actions' && tableName != 'QuotationTabSubTable'" class="mr-2">
          <CustomSpeedDial storePath="sales" :dataName="tableDataName" :rowData="slotProps.data"
        ...
        <span v-if="col.header == 'Quotation'" class="mb-1 mr-3">
          <CustomerTags marginClass="mt-3 customerTags" :label="slotProps.data[col.field]"
            :path="slotProps.data.customer_type" :tagClass="slotProps.data.customer_type" />
        </span>

        <!-- TEMPLATE : STATUS -->
        <span v-if="col.header == 'Status' && tableName == 'QuotationTabTable'" class="mr-2">
          <StatusTags :type="slotProps.data[col.field]" :label="slotProps.data[col.field]" usage="table" />
        </span>
        <!-- TEMPLATE : STATUS -->
        <span v-if="col.header == 'Status' && tableName == 'SalesTab'" class="mr-2">
          <span v-if="slotProps.data[col.field] != 'deleted'" :class="
          ...
          <span class="deletedClass" v-if="slotProps.data[col.field] == 'deleted'">
        ...
        <span v-if="col.header == 'Members'">
          ...
          <GroupedAvatars :data="
            slotProps.data[col.field].length < 3
              ? (slotProps.data[col.field].map((imageUrl) => imageUrl ? imageUrl : 'https://network-courier.sgp1.digitaloceanspaces.com/application/images/Image_1656429363602_2515_blank_profile.png'))
              : getGroupedAvatarsData(slotProps.data[col.field])
          " :more="
            slotProps.data[col.field].length >= 3
              ? slotProps.data[col.field].length - 2
        ...
        <span v-if="col.header == 'Site Office'">
          <img v-if="slotProps.data[col.field] == true" :src="'/images/' + 'siteOffice' + '.png'" alt="" />
        </span>

        <!-- TEMPLATE : Attachments -->
        <span v-if="col.header == 'Attachments'">
          <img v-if="getFilesLength(slotProps.data.attachment_url) == 1" src="/images/preview.png" alt="pdf" />
          <span class="flex" v-if="getFilesLength(slotProps.data.attachment_url) == 2">
            <img src="/images/preview.png" alt="pdf" />
            <img src="/images/preview.png" alt="pdf" />
          </span>
          <span class="flex flex-row" v-if="getFilesLength(slotProps.data.attachment_url) > 2">
            <img src="/images/preview.png" alt="pdf" />
            <img src="/images/preview.png" alt="pdf" />
    ...
    <template #expansion="slotProps">
      ...
      <span v-if="tableName == 'QuotationTabTable' && slotProps.data.revisions">
        <span v-if="slotProps.data.revisions.length != 0">
          <div class="flex flex-column mt-2" v-for="(revision, index) of slotProps.data.revisions" :key="index">
            <div class="flex ml-3">
              <StatusTags type="Revision" :label="revision[0].update + ' ' + (index + 1)" usage="table" />
            </div>
            <div :class="
            ...
            ">
              <SalesTables tableClass="p-datatable-sm QuotationTabSubTable w-100" tableDataName="QuotationTabData"
                tableName="QuotationTabSubTable" :tableData="revision" :tableColumns="
        ...
        <span v-if="slotProps.data.revisions.length == 0">
          <div class="no_data p-3">No Revisions to show</div>
        </span>
      </span>
      ...
      <span v-if="tableName == 'timeLineExpressCostCenterTable'">
        <div class="mr-2 ml-2 mt-2">
          <div class="seaDivLarge p-3">
            ...
            <TimeLineTable tableClass="p-datatable-sm TimeLineTable mt-2 mb-2" :tableData="slotProps.data.trip_schedule"
              :tableColumns="
          ...
          <div class="flex flex-row ml-5 mt-3 mb-4">
            <div v-for="(item, index) of slotProps.data.siteOfficeCharges" class="flex mr-6" :key="index">
              <div class="flex flex-column">
          ...
          <div class="flex flex-row ml-5 mb-3">
            <div class="flex" v-for="(item, index) of slotProps.data
            .siteOfficeChargesPreferences" :key="index">
    ...
    getGroupedAvatarsData(data) {
      ...
      for (let gA = 0; gA < 3; gA++) {
        if (data[gA])
          ...
          gAData.push('https://network-courier.sgp1.digitaloceanspaces.com/application/images/Image_1656429363602_2515_blank_profile.png')
      }
      return gAData;
    },
  },
