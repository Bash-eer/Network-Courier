...
export default {
  ...
  data: () => ({
    ...
    restaurant: { lat: 1.277064, lng: 103.8069639 },
    dummyCenter: {
      lat: 11.0117016,
      lng: 76.8971944,
    },
  ...
  methods: {
    ...
    setMarker(points, icons) {
      const contentString =
        ...
        '<div class="bold-600 ml-1">Name</div>' +
  ...
  watch: {
    ...
    getStoredOptimizeData: function (newVal, oldVal) {
      ...
      if (this.type === "JobTrip") {
        let positionDataLen = this.positionData.length;
        ...
        let endData =
          datLength > 0 ? dataArrays[datLength - 1]?.coordinates : null;
        let routeData = [newVal.routes].length;
  ...
  mounted() {
    ...
    if (this.type === "Rider") {
      ...
      // this.getStoredOptimizeData.coordinates?.forEach((c) => {
      //   console.log(c, '9999999999999')
      //   if (c.coordinates) {
      //     this.setMarker(c.coordinates, "/images/deliveryBoxSmall.png");
      //   }
      // });
    }
  },
