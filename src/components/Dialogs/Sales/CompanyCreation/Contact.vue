  ...
  methods: {
    ...
    submitData() {
      ...
      if (!this.v$.$invalid) {
        ...
        this.$store.state.sales.companyCreationFormData["contacts"] =
          this.details.contacts;
        //CRUD - UPDATE OPERATION
        if (this.$store.state["sales"].companyCreationStateData.id) {
          let new_contacts = [];
          ...
          if (new_contacts.length != 0) {
            let newContactsObj = {};
            newContactsObj["type_id"] =
              this.$store.state["sales"].companyCreationStateData.id;
          ...
          this.$store.state.sales.companyCreationFormData["id"] =
            this.$store.state["sales"].companyCreationStateData.id;
          //making an update/PUT api call - to update company data
          this.$store.dispatch("sales/salesCompaniesCRUD", {
            tag: "update",
            data: this.$store.state.sales.companyCreationFormData,
          });
        }
        //CRUD - CREATE OPERATION
        else {
          //making an add/POST api to create a new company
          this.$store.dispatch("sales/salesCompaniesCRUD", {
            tag: "add",
            data: this.$store.state.sales.companyCreationFormData,
          });
