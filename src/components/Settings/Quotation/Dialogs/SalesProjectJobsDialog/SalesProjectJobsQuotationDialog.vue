<template>
    ...
    <div class="formgrid grid">
      ...
      <div class="field col-12 md:col-4">
        ...
        <span v-if="v$.details['contract'][index]['email'].$error && submitted">
          <span
            id="email-error"
            v-for="(error, index) of v$.details['contract'][index]['email'].$errors"
            :key="index"
          >
        ...
        <small
          ...
          class="p-error"
          >{{
            v$.details['contract'][index]["email"].required.$message.replace("Value", "Email")
          }}</small
        >
      </div>
    </div>
  ...
  <div class="flex align-self-center justify-content-end">
    <CancelButton storePath="quotaton" />
    <Buttons
      label="Next"
...
export default {
  name: "SalesProjectJobsQuotationDialog",
  data: () => ({
    v$: useVuelidate(),
    addSalesClicked: 0,
    addSales: ["new_sales"],
    ...
    nextStepHandler() {
      ...
      if (!this.v$.$invalid) {
        ...
        this.$store.dispatch("quotation/navigateStepper", {
          step: "SalesProjectJobsSelectMethod",
        });
      }
    },
    addSale() {
