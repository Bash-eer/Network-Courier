<template>
  ...
  <div class="field col-12 md:col-12 ml-2">
    <p class="dialog-contact-header ">Service Required</p>
  </div>
...
export default {
  name: "SalesProjectJobsSelectMethod",
  data: () => ({
    v$: useVuelidate(),
    details: {
      customer_type: "",
  ...
  methods: {
    goBackHandler() {
      this.$store.dispatch("quotation/navigateStepper", {
        step: "SalesProjectJobsQuotationDialog",
      });
    },
    nextStepHandler() {
      if (this.v$.$invalid) {
      ...
      if (!this.v$.$invalid) {
        ...
        this.$store.dispatch("quotation/navigateStepper", {
          step: "SalesProjectJobsTripsDetails",
        });
      }
    },
  },
