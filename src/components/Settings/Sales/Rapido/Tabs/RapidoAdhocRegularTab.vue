...
<script>
import ContractDataTable from "../Adhoc/Tables/DataTable.vue";
...
import {
  ...
  RapidoContractUpperLevelType1Type2SubTableColumnData,
} from "../const/const";
...
export default {
  ...
  methods: {
    ...
    getTableColumnData(tab) {
      switch (tab) {
        ...
        default:
          return RapidoContractUpperLevelType1Type2SubTableColumnData;
      }
    },
    getTableData(tab) {
      switch (tab) {
  ...
  computed: {
    ...mapGetters({
      rapidoAdhocTableData: "rapidoAdhoc/rapidoAdhocTableData",
      adhocRatesSecondPageTableData:
        "rapidoAdhoc/adhocRatesSecondPageTableData",
    }),
  },
  created() {
    let ecommerceFieldData =
      this.adhocRatesSecondPageTableData[0].charge_type.find((list) => {
        if (list.e_commerce.length) {
          return list;
        }
      });
    ecommerceFieldData.e_commerce.map((listTabName) => {
      this.ecommerceTabMenuData.push({
        label: listTabName.delivery_type,
        ...listTabName,
      });
    });
  },
