...
export default {
  ...
  methods: {
    getValidationType(name) {
      ...
      let type;
      switch (name) {
        case "On Demand Service":
          type = "on_demand_services";
          break;
        case "E-Commerce":
          type = "e_commerce";
          break;
  ...
  watch: {
    ...
    "$store.state.rapidoAdhoc.addZonalRatesValidation": function () {
      if (
        this.$store.state.rapidoAdhoc.addZonalRatesValidation["details"][
          "rates"
        ][this.index]
      ) {
    ...
    value: function () {
      ...
      if (this.type != "setZonalRates") {
        this.$store.dispatch("rapidoAdhoc/" + this.type, {
