    ...
    submitData() {
      ...
      if (!this.v$.$invalid) {
        this.$store.dispatch("rapidoAdhoc/POSTPATCHCRUDOPERATION", {
          path: "/settings/sales/rates/rapido/adhoc/ratecards/clone",
          method: "POST",

          data: {
            target_rate_card_id:
              this.$store.state.rapidoAdhoc.adhocTableRowRateCardId,

            rate_card_name: this.details.cloned_rate_card_name,
          },
          toastData: {
            toastSuccessData: {
              toastMsg: `The rate card ${this.$store.state.rapidoAdhoc.adhocTableRowRateCardId} is cloned successfully`,
              toastSeverity: "info",
            },
