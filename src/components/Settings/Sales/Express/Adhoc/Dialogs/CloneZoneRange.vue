<template>
  ...
  <div class="formgrid grid">
    <div class="field col-12">
      <h5 class="required color-7a7a7a font-size-14 bold-500">Zone Range</h5>
...
export default {
  ...
  methods: {
    submitData() {
      ...
      if (!this.v$.$invalid) {
        ...
        this.$store.dispatch("expressAdhoc/POSTPATCHCRUDOPERATION", {
          path: "/settings/sales/rates/express/adhoc/ratecard/clone/zones",
          // path: "/settings/sales/rates/express/adhoc/ratecard/zones",
          method: "POST",

          data: {
            target_zone_id:
              this.$store.state.expressAdhoc.cloneZoneRangeData.id,
          ...
          loadData: {
            ...
            loadPath:
              "/settings/sales/rates/express/adhoc/ratecard/" +
    ...
    async getRateCardData() {
      await axios
        .get("/settings/sales/rates/express/adhoc/ratecard")
        .then((res) => {
          if (res.data.results) {
            let newAyyarData = [];
  ...
  async created() {
    this.details.zone_range =
      this.$store.state.expressAdhoc.cloneZoneRangeData.zone_range;
