<template>
    ...
    <div class="highlight-row ml-1 mt-3 mb-2">
      <div class="flex">
        ...
        <div class="effective-data ml-2">
          {{ dateFormater(CostingDetailsDataOne["effective_start_date"]) }}
        </div>
      </div>
  ...
  <div>
    ...
    <div class="flex flex-row flex-wrap ml-1 mt-2 mb-4">
      <div
        v-for="(item, index) of projectDetailsOne"
        :class="index != projectDetailsOne.length - 1 ? 'flex mr-6' : 'flex'"
        :key="index"
      >
        <div class="flex flex-column">
          <div class="flex van-costing-details-detailsText flex-wrap">
            {{ projectDetailsHeaderOne(index) }}
          </div>
          <div
            :class="
              item != 'High'
    ...
    <div class="highlight-row ml-1 mt-3 mb-4">
      <div class="flex">
        <div class="flex">
          ...
          <div class="effective-data ml-2">
            {{ dateFormater(projectDetailsTwo["effective_start_date"]) }}
          </div>
        </div>
        <div class="flex ml-4">
          <div class="effective-txt mb-2">Effective End Date (Tentative):</div>
          <div class="effective-data ml-2">
            {{ dateFormater(projectDetailsTwo["effective_end_date"]) }}
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-row flex-wrap">
        ...
        <div
          v-for="(item, index) of projectDetailsTwo"
          class="flex flex-wrap"
          :key="index"
        >
          <div
...
export default {
  ...
  methods: {
    ...
    async editDialog() {
      await this.setQuotationDataBikePlusProject({
      ...
      this.$store.dispatch("salesSales/openDialog", {
        dialogDetails: {
          dialogName: "ExpressContractBikeProjectJobsQuotationDialog",
          dialogHeader: "Edit",
        },
      });
