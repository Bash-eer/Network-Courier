<template>
  <div class="flex justify-content-between">
    <div
      class="
        flex
        express-project-jobs-bill-preferences-header express-international-font
      "
    >
      Bill Preferences
    </div>
    <div class="flex">
      <span
        ...
        class="express-project-jobs-bill-preferences-add"
         @click="
          openDialog(
            'AddBillPerferences',
  ...
  <hr class="express-project-jobs-bill-preferences-horizontal-line" />
  <div v-if="BillPreferencesData.length != 0" class="mt-3 mb-6">
    <p class="express-project-jobs-bill-preferences-subform-headers mb-2">
      Billing Address
    </p>
    <span class="express-project-jobs-bill-preferences-bill-details">{{
      BillPreferencesData[0].billing_address
    }}</span>
    <p class="express-project-jobs-bill-preferences-subform-headers mb-2 mt-3">
    ...
    <div class="flex flex-row">
      ...
      >
        <span v-if="invoiceNotificatons == true">
          ...
          <label
            class="express-project-jobs-bill-preferences-bill-details mt-1 ml-1"
            for="2"
...
<style scoped>
.express-project-jobs-bill-preferences-bill-details {
  color: #343434;
  font-size: 12px;
  font-weight: 700;
}
.express-project-jobs-bill-preferences-subform-headers {
  color: #4e5868;
  font-size: 12px;
  font-weight: 700;
}
.express-project-jobs-bill-preferences-horizontal-line {
  margin: 0px !important;
  margin-top: 1% !important;
}
.express-project-jobs-bill-preferences-add {
  color: blue;
  font-size: 14px;
  cursor: pointer;
}
.express-project-jobs-bill-preferences-header {
  color: #343434;
  font-size: 16px;
