<template>
  <div class="flex justify-content-between">
    <div class="flex van-bill-preference-header express-international-font">
      Bill Preferences
    </div>
    <div class="flex">
      <span
        v-if="BillPreferencesData.length == 0"
        class="van-bill-preference-add"
        @click="
          openDialog(
            'BillingPreference',
  ...
  <hr class="van-bill-preference-horizontal-line" />
  <div v-if="BillPreferencesData.length != 0" class="mt-3 mb-6">
    <p class="van-bill-preference-subFormHeaders mb-2">Billing Address</p>
    <span class="van-bill-preference-billDetails">{{
      BillPreferencesData[0].billing_address
    }}</span>
    <p class="van-bill-preference-subFormHeaders mb-2 mt-3">
    ...
    <div class="flex flex-row">
      ...
      >
        <span v-if="invoiceNotificatons == true">
          ...
          <label class="van-bill-preference-billDetails mt-1 ml-1" for="2">{{
            getLabel(index)
          }}</label>
        </span>
      </div>
...
export default {
  ...
  methods: {
    openDialog(name, type, header) {
      ...
      if (type == "salesExpressContractVan") {
        this.$store.dispatch("salesExpressContractVan/openDialog", {
          dialogDetails: {
            dialogName: name,
            dialogHeader: header,
          },
        });
        this.$store.getters["salesExpressContractVan/controlActivityDialog"];
      }
    },
...
<style scoped>
.van-bill-preference-billDetails {
  color: #343434;
  font-size: 12px;
  font-weight: 700;
}
.van-bill-preference-subFormHeaders {
  color: #4e5868;
  font-size: 12px;
  font-weight: 700;
}
.van-bill-preference-horizontal-line {
  margin: 0px !important;
  margin-top: 1% !important;
}
.van-bill-preference-add {
  color: blue;
  font-size: 14px;
  cursor: pointer;
}
.van-bill-preference-header {
  color: #343434;
  font-size: 16px;
}
</style>
