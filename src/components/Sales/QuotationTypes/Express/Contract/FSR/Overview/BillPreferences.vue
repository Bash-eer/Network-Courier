<template>
  <div class="flex justify-content-between">
    <div class="flex fsr-header express-international-font">
      Bill Preferences
    </div>
    <div class="flex">
      <span
  ...
  <hr class="fsr-horizontal-line" />
  <div v-if="BillPreferencesData.length != 0" class="mt-3 mb-6">
    <p class="fsr-sub-form-headers mb-2">Billing Address</p>
    <span class="fsr-billDetails">{{
      BillPreferencesData[0].billing_address
    }}</span>
    <p class="fsr-sub-form-headers mb-2 mt-3">Invoice Notification</p>
    <div class="flex flex-row">
      ...
      >
        <span v-if="invoiceNotificatons == true">
          ...
          <label class="fsr-billDetails mt-1 ml-1" for="2">{{
            getLabel(index)
          }}</label>
        </span>
      </div>
    </div>
    <p class="fsr-sub-form-headers mb-2 mt-3">Invoice Attachments</p>
    <div class="flex flex-row">
      <div
        class="flex mr-3"
    ...
    <p class="fsr-sub-form-headers mb-2 mt-3">Attachment Format</p>
    <div class="flex flex-row">
      <div
        class="flex mr-3"
  ...
  <Dialog
    ...
    :dialogComponent="$store.state['salesExpressContractFSR'].dialogComponent"
  />
</template>
...
export default {
  ...
  methods: {
    ...
    openDialog(name, type, header) {
      ...
      if (type == "salesExpressContractFSR") {
