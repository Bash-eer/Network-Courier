<template>
  <div class="flex flex-row justify-content-between align-items-center">
    <div class="font-size-16 bold-600 color-343434 express-international-font">
      Follow Up
    </div>
    <div
      class="blue pointer-cursor title mr-3"
      id="foo"
      @click="openDialog('FollowUp', 'salesExpressAdhoc', 'Add Follow Up')"
    >
      +Add New
    </div>
  </div>
...
export default {
  ...
  methods: {
    openDialog(name, type, header) {
      ...
      if (type == "salesExpressAdhoc") {
        this.$store.dispatch("salesExpressAdhoc/openDialog", {
          dialogDetails: {
            dialogName: name,
            dialogHeader: header,
          },
        });
        this.$store.getters["salesExpressAdhoc/controlActivityDialog"];
      }
    },
  },
};
