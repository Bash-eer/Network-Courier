<template>
  ...
  >
    ...
    <template #expansion="slotProps">
      ...
      <span v-if="tableName == 'timeLineRapidoContractCostCenterTable'">
        <div class="mr-2 ml-2 mt-2">
          <div class="international-contract-data-table-seadivlarge p-3">
            <div class="flex flex-wrap justify-content-between">
              <div class="flex">
                <span class="header">Pick Up Locations</span>
              </div>
...
export default {
  ...
  methods: {
    ...mapActions({
      costCenterById: "salesRapidoContract/Contract/costCenterById",
    }),
    ...
    async onRowExpand(event) {
      ...
      const res = await this.costCenterById({
        type: this.$route.params.type,
        id: event.data.id,
      });

      if (res) {
        this.expandedData = res.data.results?.pick_up_locations;

        console.log(this.expandedData, "esp");
      }
    },
    ...
    onRowSelect(event) {
      if (this.tableName == "QuotationTabTable") {
        ...
        this.$store.state.salesRapidoContract.internationalRegionsTripTableData =
          null;

        this.$store.state.salesRapidoContract.rapidoQuotationOverviewData = [];
        ...
        this.$store.state.salesRapidoContract.rapidoQuotationOverviewData.push(
          overviewDetailsObj
        );
        ...
        this.$store.dispatch("salesRapidoContract/loadBillPreferences");
        this.$store.dispatch("salesRapidoContract/loadCostCentres");
        this.$store.dispatch("salesRapidoContract/loadAttachments");
        this.$store.dispatch("salesRapidoContract/loadFollowUp");
      }
      if (this.tableName == "SalesTab") {
        ...
        this.$store.dispatch("salesRapidoContract/loadContactsApi", {
          apiPath: apiPath,
        });
        this.$store.dispatch("salesRapidoContract/loadQuotationApi", {
          id: event.data.id,
        });
