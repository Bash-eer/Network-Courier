...
<template>
  ...
  >
    ...
    <Column
      ...
      style="width: 25%"
      class="EditTable"
      :sortable="col.header != 'S. No' && col.header != 'Remove' ? true : false"
      :showFilterMenu="
        col.header != 'S. No' && col.header != 'Remove' ? true : false
      "
    ...
    >
      ...
      >
        ...
        <span v-if="col.header == 'Increment Percentage (%)'" key="renderKey">
          <InputNumber
            v-model="data[field]"
            :showButtons="true"
            mode="decimal"
        ...
        <span v-if="col.header == 'Remarks'" key="renderKey">
...
export default {
  ...
  methods: {
    ...
    removeRow(event) {
      ...
      this.$confirm.require({
        ...
        accept: () => {
          this.$store.state.bulkAdjustment.normalTableRowDelete = {};
          this.$store.state.bulkAdjustment.normalTableRowDelete = event;
        },
      });
      // for (let i in this.tableData) {
      //   if (this.tableData[i].id == event.data.id) {
      //     console.log("Deleted", event.data);
      //   } else {
      //     console.log("Table data", this.removedTableData);
      //     this.removedTableData[i] = this.tableData[i];
      //   }
      //   this.bulkAdjustmentAddNewData.profiles.normal = this.removedTableData;
      // }
    },
    onCellEditComplete(event) {
      ...
      if (event.field == "Increment_amount") {
        ...
        this.bulkAdjustmentAddNewData.profiles.normal[i].Increment_Percentage =
          Number(
            (
        ...
        this.bulkAdjustmentAddNewData.profiles.normal[i].new_cost =
          this.bulkAdjustmentAddNewData.profiles.normal[i].Increment_amount +
          this.bulkAdjustmentAddNewData.profiles.normal[i].per_job;
      } else if (event.field == "Increment_Percentage") {
        let i = event.index;
        this.bulkAdjustmentAddNewData.profiles.normal[i].Increment_amount =
          Number(
            (
        ...
        this.bulkAdjustmentAddNewData.profiles.normal[i].new_cost =
          this.bulkAdjustmentAddNewData.profiles.normal[i].Increment_amount +
          this.bulkAdjustmentAddNewData.profiles.normal[i].per_job;
        // console.log(this.bulkAdjustmentAddNewData.profiles.normal[i], "amount");
      }
      //
      this.$store.state.bulkAdjustment.editField = event.field;
      ...
      this.$store.state.bulkAdjustment.rowEdit["remarks"] = event.data.remarks;
    },
  ...
  watch: {
    ...
    "$store.state?.bulkAdjustment?.EcomProfileData[0]?.selected":
      function () {},
