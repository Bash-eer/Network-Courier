<template>
  ...
  <div class="row ml-1 table_div">
    <div class="col-12">
      <FormTable
        :tableData="$store.state.dialog.expressQuotationContactsTableData"
        :tableColumns="
          $store.state.dialog.expressQuotationContactsTableColumnData
        "
      />
    </div>
  </div>
...
export default {
  name: "SalesExpressContractInterChargesFifthStep",
  ...
  methods: {
    goBackHandler() {
      this.$store.dispatch("salesQuotationDialog/navigateStepper", {
        step: "SalesExpressContractInterChargesFourthStep",
      });
    },
    submitData() {
      if (this.v$.$invalid) {
