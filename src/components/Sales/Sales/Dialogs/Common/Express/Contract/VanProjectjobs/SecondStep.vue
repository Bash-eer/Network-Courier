  ...
  <div class="formgrid grid ml-3">
    ...
    <div class="field col-12 md:col-6">
      ...
      <DropDown
        ...
        :key="$store.state.salesQuotationDialog.selectDropDown"
...
export default {
  name: "SalesExpressContractVanProjectJobsSecondStep",
  data: () => ({
    v$: useVuelidate(),
    details: {
      customer_type: "",
  ...
  methods: {
    goBackHandler() {
      this.$store.dispatch("salesQuotationDialog/navigateStepper", {
        step: "SalesExpressContractVanProjectJobsFirstStep",
      });
    },
    nextStepHandler() {
      if (this.v$.$invalid) {
      ...
      if (!this.v$.$invalid) {
        this.$store.dispatch("salesQuotationDialog/navigateStepper", {
          step: "SalesExpressContractVanProjectJobsThirdStep",
        });
      }
    },
  },
