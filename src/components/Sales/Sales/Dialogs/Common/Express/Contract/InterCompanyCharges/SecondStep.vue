<template>
  ...
  <div class="formgrid grid ml-3">
    <div class="field col-12 md:col-6">
      <h5 class=" dialog-label-text required">Referred By</h5>
      <DropDown
        label="referred_by"
    ...
    <div class="field col-12 md:col-6">
      <h5 class=" dialog-label-text required">Referral Person Name</h5>
      <DropDown
...
export default {
  name: "SalesExpressContractInterChargesSecondStep",
  data: () => ({
    v$: useVuelidate(),
    details: {
      customer_type: "",
  ...
  methods: {
    goBackHandler() {
      this.$store.dispatch("salesQuotationDialog/navigateStepper", {
        step: "SalesExpressContractInterChargesFirstStep",
      });
    },
    nextStepHandler() {
      if (this.v$.$invalid) {
