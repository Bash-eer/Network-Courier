<template>
  <div>
    ...
    <div class="table_component" :key="renderkey">
      <BikeTable
        :tableData="tabledata"
...
import BikeTable from "./checkInDataTable.vue";
import { tableColumns } from "./const";
import { mapActions, mapMutations } from "vuex";
export default {
  components: { Buttons, BikeTable },
  data() {
    return {
      tableColumns,
      scan_no: "",
  ...
  methods: {
    ...mapActions({
      getJobIdAndLoadData: "CheckInTab/getJobId",
      updateJobsForCheckIn: "CheckInTab/updateJobsForCheckIn",
      printMovementChart: "CheckInTab/PrintMovementChart",
    }),
    ...mapMutations({
      clearDataInCheckInStore: "CheckInTab/clearJobsData",
    }),

    filterKeys(arr) {
      // let formatted = arr;
      let fields = ["id", "selection", "remarks", "job_type"];

      let formatted = arr.map((obj, index) => {
        Object.keys(obj).forEach((ele) => {
          if (fields.includes(ele)) {
            delete arr[index][ele];
          }
        });
      });
      return formatted;
    ...
    removeAndFocusScanAndJobNo(field) {
      if (field == "scan_no") {
        this.scan_no = null;
    ...
    async printChart() {
      if (this.mChart != null) {
        let response = await this.printMovementChart(this.mChart);
        if (response && response.status === 200) {
          ...
          let info =
            ...
            "There was an error in creating a preview, try again!";
    ...
    async submitData() {
      const res = await this.updateJobsForCheckIn(this.formData);
      if (res && res.status == 200) {
        const successMessage =
          res?.results || "Check-In Details Updated Successfully";
