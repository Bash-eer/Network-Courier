      ...
      <div class="flex ml-4 align-self-center">
        <div class="flex flex-row flex-wrap">
          <div class="flex tabPills mr-6">
            <!-- tab header title-->
            <span class="table-title">Check In</span>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="flex tabPills mr-6">
          ...
          >
            <li
              class="nav-item"
              role="presentation"
              v-for="(tabs, index) of checkInTabData"
              :key="tabs.label"
            >
              <button
...
<script>
import { checkInTabData } from "./const";
...
export default {
  ...
  methods: {
    ...mapActions({ getRemarksDropdown: "CheckInTab/getRemarksData" }),
    async tabChange(tab) {
