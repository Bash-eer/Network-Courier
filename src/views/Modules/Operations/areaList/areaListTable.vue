<template>
  <DataTable
    :loading="$store.state['zoneAreaList'].tableLoad"
    :class="tableClass"
    :showGridlines="false"
    :stripedRows="true"
    :rowClass="rowClass"
    ...
    :rows="$store.state['zoneAreaList'].pageLimit"
  ...
  >
    ...
    >
      <template #body="slotProps">
        ...
        <span v-if="col.header == 'Area Name'" class="mr-2">
          {{ slotProps.data?.area_name || "-" }}
        </span>

        <span v-if="col.header == 'Postal Code'" class="mr-2">
          {{ slotProps.data?.from_pincode || "-" }}
        </span>

        <span v-if="col.header == 'Unit No' && slotProps.data['unit_no']">
          <i class="pi pi-check zone-address-check-icon"></i>
        </span>

        <span v-if="col.header == 'Floor No' && slotProps.data['floor_no']">
          <i class="pi pi-check zone-address-check-icon"></i>
        </span>
  ...
  methods: {
    ...
    initilaizeFilters() {
      ...
      let permanentTablelColumns = JSON.parse(
        localStorage.getItem("areaListColumns")
      );
    ...
    commonPageChangeLoad(paginationParams) {
      ...
      let data = {
        ...
        filterParams: {
          orderBy: paginationParams.filterParams.orderBy,
          filter_params: this.filteredItems,
        },
      };
      this.getAllAreaList(data);
    },
    onToggle(value) {
      localStorage.setItem("areaListColumns", JSON.stringify(value));
