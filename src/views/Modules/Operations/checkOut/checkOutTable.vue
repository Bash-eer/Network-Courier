        ...
        <span v-if="col.header == 'Remarks'">
          <span v-if="data.selection && data.selection == true">
            <Dropdown
              ...
              optionValue="code"
              class="w-12rem ml-1"
              @change="updateRemarksData($event, index, 'remarks')"
              :filter="true"
            >
            </Dropdown>
          </span>
...
import checkoutPrintDialog from "./dialog/checkoutPrintDialog.vue";
import checkoutDialogPreview from "./dialog/checkoutDialogPreview.vue";
...
export default {
  ...
  computed: {
    ...mapState({
      remarksDropdownData: (state) => state.Checkout.remarksDropdownData,
      jobsData: (state) => state.Checkout.jobsData,
      totalJobsData: (state) => state.Checkout.totalJobsData,
      selectedJobsData: (state) => state.Checkout.selectedJobsData,
      totalJobsSelected: (state) => state.Checkout.totalJobsSelected,
    }),
  },
  methods: {
    ...mapActions({
      showCheckoutPrintPreviewDialog: "Checkout/showPrintPreviewDialog",
    }),

    ...mapMutations({
      updateSelectionMode: "Checkout/updateSelectionMode",
      clearJobsData: 'Checkout/clearJobsData',
      clearSelectionJobsData: 'Checkout/clearSelectionJobsData'
    }),
    ...
    showPrintLabelDialog(data) {
      ...
      this.initialComponent = "checkoutPrintDialog";
      this.dialogComponent = "checkoutDialogPreview";
    ...
    updateRemarksData({value}, index, key) {
    ...
    checkAndPushTableData(){
      if (this.jobsData && this.totalJobsData && this.tabledata != null) {
          ...
          this.tabledata =
            this.jobsData == null ? [] : this.jobsData;
        }
    },
