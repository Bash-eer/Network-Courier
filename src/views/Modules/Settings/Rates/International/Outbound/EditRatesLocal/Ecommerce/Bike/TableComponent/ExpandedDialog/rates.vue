...
export default {
  ...
  methods: {
    ...mapActions({
      postBikeDtRates: "Outbound/Edit/postBikeDtRates",
      editBikeDtRates: "Outbound/Edit/editBikeDtRates",
    }),
    async save() {
      if (this.v$.$invalid) {
        this.isValid = false;
      ...
      if (this.mode === "Edit") {
        ...
        try {
          this.isloading = true;
          await this.postBikeDtRates({
            delivery_type_id: this.local_edit.type_id_2,
            ...this.formData,
          });
          this.$emit("close");
          this.$emit("save");
