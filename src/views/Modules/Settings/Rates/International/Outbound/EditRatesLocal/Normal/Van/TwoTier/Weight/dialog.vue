<template>
  ...
  >
    <template #header>
      <div v-if="editData?.type == 'normal_van_twotier_w'">Edit</div>
      <div v-else>Add New</div>
    </template>
    <div class="content">
      <div class="input-wrapper col">
        <div class="label flex align-items-start mb-2">Maximum Weight (Kg)</div>
        <NumberField
...
export default {
  ...
  methods: {
    ...mapActions({
      editChargeByWeight: "Outbound/Edit/editChargeByWeight",
    }),
    async save() {
      if (this.v$.$invalid) {
        this.isValid = false;
      ...
      if (this.editData?.type == "normal_van_twotier_w") {
        try {
          this.isloading = true;
          await this.editChargeByWeight({
