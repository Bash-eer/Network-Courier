<template>
  <div>
    <DataTable
      tableName="transitDocumentTable"
      :tableData="tableData"
    ...
    />
    <EditDialog
      v-if="displayDialog.displayDocumentDialog"
      :show="displayDialog.displayDocumentDialog"
      @close="closeDialog"
      @save="renderKey++"
    />
  </div>
...
export default {
  ...
  methods: {
    ...
    closeDialog() {
      this.setTransitDaysDialog({ type: "displayDocumentDialog", data: false });
    },
    async getData() {
      try {
        this.tableData = [];
