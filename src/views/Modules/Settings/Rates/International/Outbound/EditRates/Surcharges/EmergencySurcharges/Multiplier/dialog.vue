<template>
  ...
  >
    <template #header>
      <div v-if="editData?.type == 'emergency_m'">Edit</div>
    ...
    <div class="content">
      ...
      <div class="flex align-items-end">
        <div class="input-wrapper col mt-3">
          <div class="label flex align-items-start mb-2">Per Kg Doc $</div>
          <NumberField
            v-on:childToParent="getUserInput"
            label="document"
            class="number-field"
        ...
        <div class="input-wrapper col mt-3">
          ...
          <NumberField
            v-on:childToParent="getUserInput"
            label="document_min_rate"
            class="number-field"
            v-model="formData.rates.document_min_rate"
            mode="decimal"
            :maxFractionDigits="2"
            :classes="{
              'p-invalid':
                v$.formData.rates.document_min_rate.$invalid && !isValid,
            }"
          />
        </div>
      </div>
      <div class="flex align-items-end">
        ...
        <div class="input-wrapper col mt-3">
          ...
          <NumberField
            v-on:childToParent="getUserInput"
            label="parcel_min_rate"
            class="number-field"
            v-model="formData.rates.parcel_min_rate"
            mode="decimal"
            :maxFractionDigits="2"
            :classes="{
              'p-invalid':
                v$.formData.rates.parcel_min_rate.$invalid && !isValid,
            }"
          />
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...mapActions({
      ...
      postEmergencyCharges: "Outbound/Edit/postEmergencyCharges",
      editEmergencyCharges: "Outbound/Edit/editEmergencyCharges",
    }),
    async save() {
      console.log(this.v$.$invalid, "valie", this.v$.formData.rates.$invalid);
      if (this.v$.formData.rates.$invalid) {
        this.isValid = false;
        return;
      }

      if (this.editData?.type == "emergency_m") {
        try {
          this.isloading = true;
          await this.editEmergencyCharges({
            type: "multiplier",
        ...
        try {
          this.isloading = true;
          await this.postEmergencyCharges({
            type: "multiplier",
            data: {
              type_id: this.imp_edit.type_id,
              ...this.formData.rates,
            },
          });
          this.$emit("close");
  ...
  async created() {
    ...
    if (this.editData?.type == "emergency_m") {
      ...
      this.formData.rates.parcel_min_rate = data.parcel_min_rate;
      this.formData.rates.document_min_rate = data.document_min_rate;
    }
  },
};
</script>
