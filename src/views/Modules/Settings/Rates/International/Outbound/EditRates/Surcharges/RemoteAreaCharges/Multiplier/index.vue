<template>
  <div>
    <DataTable
      tableName="remoteChargeMultiplierTable"
      :tableData="tableData"
      :tableColumns="tableColumns"
      :expansion="false"
    />
    <Dialog
      :show="imp_edit.remote_area_charges_m"
      v-if="imp_edit.remote_area_charges_m"
      @close="closeDialog"
      @save="renderKey++"
    />
  </div>
...
import { surchargesRemoteareaMultiplierColumns } from "../../../../const";
...
export default {
  ...
  data() {
    return {
      renderKey: 10,
      tableData: null,
      tableColumns: surchargesRemoteareaMultiplierColumns,
    };
  },
  watch: {
    renderKey() {
      ...
      if (response) {
        ...
        response.data.results.surcharges.map((list) => {
          if (list.type == "remote_area_charges") {
            ...
            list.remote_area_multiplier_rates.map((rates, idx) => {
              data.push({
                sl_no: idx + 1,
                ...
                per_document_rate: rates.per_document_rate,
                per_parcel_rate: rates.per_parcel_rate,
                postal_code: rates.postal_code,
                per_parcel_min_rate: rates.per_parcel_min_rate,
                per_document_mim_rate: rates.per_document_mim_rate,
