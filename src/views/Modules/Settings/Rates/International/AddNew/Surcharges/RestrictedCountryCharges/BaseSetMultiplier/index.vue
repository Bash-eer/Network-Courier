  ...
  <div>
    <div v-for="(data, idx) of rowData" :key="data">
      <InputRow
        ...
        @removeRow="removeRow"
        type="base_rate_multiplier"
        stage="Base Set Multiplier"
      />
    </div>
...
export default {
  ...
  methods: {
    ...
    addRow() {
      ...
      this.addRestrictedCountry("base_rate_multiplier");
    },
    removeRow(id) {
      if (this.rowData.length > 1) {
        this.rowData.splice(id, 1);
        this.removeRestrictedCountry({ type: "base_rate_multiplier", idx: id });
      }
    },
  ...
  created() {
    this.restrictedCountry.base_rate_multiplier.map((_, idx) => {
