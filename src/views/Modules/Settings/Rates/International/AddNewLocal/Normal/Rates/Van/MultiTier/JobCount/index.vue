...
import JobCountInputs from "./JobCountInputs.vue";
export default {
  components: { JobCountInputs },
  data() {
    return {
      rowData: [],
    };
  ...
  methods: {
    ...mapActions({
      addMultiTierJobCount:
        "AddNewInternationalLocal/Normal/addMultiTierJobCount",
      removeMultiTierJobCount:
        "AddNewInternationalLocal/Normal/removeMultiTierJobCount",
    ...
    addRow() {
      ...
      this.addMultiTierJobCount();
    },
    removeRow(id) {
      if (this.rowData.length > 1) {
        this.rowData.splice(id, 1);
        this.removeMultiTierJobCount(id);
      }
    },
  ...
  created() {
    this.multi_tire.charge_by_job_count.map((_, idx) => {
