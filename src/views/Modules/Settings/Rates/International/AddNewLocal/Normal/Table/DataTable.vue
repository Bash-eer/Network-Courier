...
export default {
  ...
  watch: {
    ...
    selectedRows(value) {
      let rowId = [];
      value.map((list) => rowId.push({ region_trip_id: list.region_trip_id }));
      this.setTrips({ type: this.$props.type, data: rowId });
  ...
  methods: {
    ...mapActions({
      setTrips: "AddNewInternationalLocal/Normal/setTrips",
      setValidations: "AddNewInternationalLocal/Normal/setValidations",
    }),
    prefillData() {
      if (this.trips[this.$props.type]) {
        this.selectedRows = this.trips[this.$props.type];
      }
    },
  },
  mounted() {
