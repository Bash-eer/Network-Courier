...
export default {
  ...
  methods: {
    ...mapActions({
      AddVanDelivery: "AddNewInternationalLocal/Ecommerce/AddVanDelivery",
      removeVanDelivery: "AddNewInternationalLocal/Ecommerce/removeVanDelivery",
    ...
    prefillData() {
      this.formData.timeslot_delivery = this.van?.timeslot_delivery || "";
      this.formData.timeslot_delivery_fixed_hours =
        this.van?.timeslot_delivery_fixed_hours || "";
    },
  },
  mounted() {
    this.prefillData();
  ...
  created() {
    this.van.delivery_type.map((_, idx) => {
