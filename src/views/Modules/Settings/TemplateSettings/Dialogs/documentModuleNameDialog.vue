<template>
  <div v-for="(module, index) of addModules" :key="module">
    ...
    <div class="formgrid grid mb-3">
      <div class="field col-12 md:col-12">
        <h5 class="required color-7a7a7a font-size-14 bold-500">Module Name</h5>
        <TextField
  ...
  <div v-if="!documentmoduleEditData.id" class="flex mb-2">
...
export default {
  ...
  computed: {
    ...mapGetters({
      documentmoduleEditData: "templateSettings/documentmoduleEditData",
    }),
  },
  methods: {
    closeDialog() {
    ...
    submitData() {
      ...
      if (!this.v$.$invalid) {
        if (this.documentmoduleEditData.id) {
    ...
    apiCallHandler() {
      this.$store.dispatch("templateSettings/POSTPATCHCRUDOPERATION", {
        path: "/settings/templates/document/module",
        data: this.details,
        method: "POST",
    ...
    apiPatchCallHandler() {
      this.$store.dispatch("templateSettings/POSTPATCHCRUDOPERATION", {
        path:
          ...
          this.documentmoduleEditData.id,
  ...
  created() {
    if (this.documentmoduleEditData) {
      this.details.modules[0].module_name =
        this.documentmoduleEditData.module_name;
    }
  },
  unmounted() {
