<template>
  <main>
    <div id="app" class="flex flex-row" @drop.prevent>
      <div
        class="container"
      ...
      >
        ...
        >
          ...
          <div class="flex flex-row">
            <div class="roundPin">
              <img class="cursor mx-3" src="/images/pin.png" alt="" />
            </div>
            <div class="flex flex-column mt-2 mx-3">
              <span class="drag-text pt-1">Upload template</span>
      ...
      <!--for only one file-->
      <div v-if="filesToUpload.mime_type">
        ...
        >
          <img
            v-if="filesToUpload.mime_type == 'image/png'"
            src="/images/drag-drop-img.png"
            alt=""
          />
          <img
            v-else-if="filesToUpload.mime_type == 'application/pdf'"
            src="/images/drag-drop-pdf.png"
            alt=""
          />
          <img
            v-else-if="
              filesToUpload.mime_type ==
          ...
          <div class="flex flex-column align-items-start pl-2">
            <div class="drag-title">{{ this.filesToUpload.file_name }}</div>
...
export default {
  ...
  methods: {
    ...
    getBasePath(type) {
      let path;
      switch (type) {
        case "templateSettingsAttachments":
          path = "templateSettingsAttachments";
          break;
      }
      return path;
    },
    getDeletedIndicesArray(type) {
      ...
      switch (type) {
        case "templateSettingsAttachments":
          deletedIndicesArray = "deletedtemplateSettingsAttachmentsFiles";
          break;
      }
    ...
    async getFileURL(f, index) {
      ...
      axios
        ...
        .then(async (res) => {
          ...
          this.$store.state.templateSettings.templateSettingsAttachmentsFiles =
            this.filesToUpload;
          this.$store.state.templateSettings.attachURL = this.fileURL;
  ...
  updated() {
    ...
    if (path == "templateSettingsAttachments") {
      this.$store.state.templateSettings.attachURL = this.fileURL;
      this.$store.state.templateSettings.templateSettingsAttachmentsFiles =
        this.filesToUpload;
    }
